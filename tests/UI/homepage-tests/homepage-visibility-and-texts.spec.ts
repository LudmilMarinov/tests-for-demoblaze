...
test("Verify homepage categories count, texts and visibility", async ({ homePage }) => {
    const allCategoriesLocators = await homePage.categoryComponent.linkComponent.all();

    const allCategoriesTexts = await Promise.all(allCategoriesLocators.map((locator) => locator.innerText()));
...
test("Verify each product card has a price tag visible", async ({ homePage }) => {
