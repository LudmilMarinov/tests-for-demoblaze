...
test("Verify logged user can add product to cart and product added alert is present", async ({ homePage, productPage }) => {
    await homePage.login(createUserData.username, createUserData.password);
    await homePage.productGrid.getProductCardByCardTitle(addProductTestData.productName).productCardInfoBlock.title.click();
    const [dialog] = await Promise.all([productPage.page.waitForEvent("dialog"), productPage.productContentComponent.productContentBody.addToCartButton.click()]);
